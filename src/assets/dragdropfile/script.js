...
//	template += '<div class="progress" id="'+rand+'"><span></span></div>';
  template += '<div class="result" id="'+rand+'"><span></span></div>';
...
/*  $.ajax({
    type:'POST',
    url: 'http://54.208.187.238/Camera51Server/uploadimage',
    data: formData,
    cache: false,
    contentType: false,
    processData: false,
  ...
  return;*/
	...
	xhr[rand].onreadystatechange = function (oEvent) {
      ...
      console.log("done",xhr[rand].responseText);
      uploadDone(rand,xhr[rand].responseText );
		} else {
